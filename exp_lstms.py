...
def main():
    ...
    train_dataset = FunctionsDataset("data/functions", "train.txt", "singles", "tokens")
    num_tokens = train_dataset.num_tokens
    val_dataset = FunctionsDataset("data/functions", "val.txt", "singles", "tokens")
    ...
    val_triplets = torch.tensor(samplers.FixedSetOfTriplets(val_dataset.labels, 10**3).fixed_set_of_triplets)
    ...
    train_loader = DataLoader(train_dataset, num_workers=16, pin_memory=True, collate_fn=Collater(), sampler=RandomSampler(train_dataset, replacement=True, num_samples=2000), batch_size=batch_size)
    val_loader = DataLoader(val_dataset, num_workers=16, pin_memory=True, collate_fn=Collater(), batch_size=batch_size)

    encoder = Classifier(LSTMEncoder(num_tokens, 8, 512, 3))
    encoder.to(device)
    # Set optimizers
    ...
    for epoch in range(1, epochs + 1):
        ...
        if acc < best_acc:
            best_acc = acc
            no_improvement_since = 0
            torch.save(encoder.state_dict(), "exp_lstm_best.pt")
        else:
